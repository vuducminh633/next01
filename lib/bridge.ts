...
export async function runCppConverter(cadData: any[]) {
  ...
  try {
    ...
    const processedData = cadData.map((obj, index) => {
      ...
      // SAFTEY FIX: Some C++ files look for "PartType", some look for "Type". 
      // We will force BOTH to exist so the C++ doesn't crash trying to read a missing key!
      ...
      return { 
        ...
        PartType: safeType,    // Guarantee this exists too
